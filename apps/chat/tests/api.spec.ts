...
describe('sendMessage SSE parsing', () => {
  ...
  it('dispatches every frame once, across chunk boundaries mid-line', async () => {
    ...
    await sendMessage('s', 'hi', undefined, undefined, (event) => { events.push(event) })
  ...
  it('surfaces the error body of a rejected send', async () => {
    ...
    await expect(sendMessage('s', '', undefined, undefined, () => undefined)).rejects.toThrow('text must be a non-empty string')
  })

  it('throws on a missing stream body', async () => {
    stubFetch(() => ({ ok: true, status: 200, statusText: 'OK', body: null } as unknown as Response))
    await expect(sendMessage('s', 'hi', undefined, undefined, () => undefined)).rejects.toThrow('200 OK')
  })
})
...
describe('sendMessage reply context', () => {
  it('rides the reply target in the POST body and omits it when absent', async () => {
    const calls: unknown[] = []
    stubFetch((_url, init) => {
      calls.push(JSON.parse(String(init?.body)))
      return okResponse(sseBody(['data: {"t":"turn-end","reason":"completed"}\n\n']))
    })
    await sendMessage('s', 'same to you', undefined, { role: 'assistant', text: 'hello' }, () => undefined)
    await sendMessage('s', 'plain', undefined, undefined, () => undefined)
    expect(calls).toEqual([
      { text: 'same to you', replyTo: { role: 'assistant', text: 'hello' } },
      { text: 'plain' },
    ])
  })
})
...
describe('sendMessage callback failures', () => {
  it('propagates a throwing callback instead of swallowing it as a malformed frame', async () => {
    ...
    await expect(sendMessage('s', 'hi', undefined, undefined, () => { throw new Error('render exploded') }))
